...
{ SUBJECT }
Manage my photo gallery
{ ENDSUBJECT }

{ TEXTBODY }
%(user.display_name)s has created a photo gallery on Zoto and invites you to add your pictures to it and be an administrator as well!
...
http://%(browse_username)s.%(domain)s/galleries/%(gallery)s/?p=%(password_hash)s
...
<head>
  ...
  <title>Manage my photo gallery</title>
</head>
<body>
...
<p>
<a href="http://%(browse_username)s.%(domain)s/galleries/%(gallery)s/?p=%(password_hash)s">
http://%(browse_username)s.%(domain)s/galleries/%(gallery)s/?p=%(password_hash)s</a>
</p>
<p>
